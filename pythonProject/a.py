...
client = QdrantClient(
    url='https://0d9e031b-a61d-479b-8a5e-1b1aeeba93a0.us-west-1-0.aws.cloud.qdrant.io:6333',
    api_key='eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJhY2Nlc3MiOiJtIn0.kVTMd_unNC9-pK_NsxehcjvoHSVzeHT7OmmmCdgppis',
)

old_collection = "Sổ tay sinh viên"
new_collection = "so_tay_sinh_vien_2024"

# 1. Lấy schema của collection cũ
schema = client.get_collection(old_collection).config

# 2. Tạo collection mới với schema tương tự
client.recreate_collection(
    collection_name=new_collection,
    vectors_config=schema.params.vectors
)

# 3. Lấy toàn bộ dữ liệu từ collection cũ
scroll_iter = client.scroll(old_collection, limit=330)
while scroll_iter[0]:
    points = scroll_iter[0]
    # Thêm điểm vào collection mới
    client.upsert(
        collection_name=new_collection,
        points=[PointStruct(id=p.id, vector=p.vector, payload=p.payload) for p in points]
    )
    scroll_iter = client.scroll(old_collection, offset=scroll_iter[1], limit=100)

# 4. (Tuỳ chọn) Xoá collection cũ
