...
def predict_question_belong_to_stsv():
    ...
    Cypher query:
    MATCH p=(predict:<type> {{name: '<<điền đầy đủ name từ mục lục>>'}})-[r*1..2]->(e)
    RETURN p

    Trong đó:
        - <type>: Là loại của mục (Part, Section, hoặc Article phải ghi hoa chữ cái đầu).
        - <<điền đầy đủ name từ mục lục>>: Là nội dung của mục name trong mục lục (viết thường, tiếng Việt, đúng như trong mục lục).
...
# yêu cầu llm dự đoán câu hỏi sẽ thuộc về phần nào trong sổ tay sinh viên
def predict_question_belong_to_stsv_01():
    return """
...
Nhiệm vụ của bạn:
Các câu trả lời sẽ được cung cấp theo nhiều bước. Bạn cần chọn ra những bước có chứa thông tin liên quan và chỉ đưa ra câu trả lời tổng hợp cuối cùng thỏa mãn tất cả các điều kiện nêu trong câu hỏi ban đầu.
Không đưa ra bất kỳ giải thích hoặc lập luận nào. Bạn có thể sử dụng kiến thức của mình đề có thể bổ sung thêm thông tin cho câu trả lời phong phú hơn. Câu trả lời phải ghi đúng chính tả và tự nhiên theo tiếng việt
...
Với liên kết và URL, không cần in đậm.
Nếu có nhiều mục, hãy liệt kê theo thứ tự và in đậm các thuật ngữ chính.
"""
...
Nếu tài liệu không có thông tin bạn có thể sử dụng kiến thức của mình để có thể bổ sung thêm thông tin cho câu trả lời phong phú hơn(nếu có câu trả lời) hoặc có thể trả lời(nếu không có tài liệu tham khảo nào). Câu trả lời phải ghi đúng chính tả và tự nhiên theo tiếng việt
